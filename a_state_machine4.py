...
def calculate_distance():
    ...
    left_distance = left_encoder - previous_left_encoders
    right_distance = right_encoder - previous_right_encoders

    robot_angle2 = math.radians(robot_angle)  # Convert degrees to radians
    x -= math.sin(robot_angle2)*left_distance
    y += math.cos(robot_angle2)*left_distance
...
def forward():
    ...
    display.fill(0)
    display.text("Moving Forward", 0, 40)
    display.show()
...
def backward():
    ...
    motors.set_right_speed(-max_speed)
    display.fill(0)
    display.text("Moving Backword", 0, 40)
    display.show()  
...
while True:
    ...
    display.fill(0)
    display.text(f"x:{x:>8}", 50, 26)
    display.text(f"y:{y:>8}", 50, 38)
    display.show()
